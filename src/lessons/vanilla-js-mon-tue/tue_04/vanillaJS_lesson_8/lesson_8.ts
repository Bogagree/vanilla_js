...
export function getTriangleType(a: number, b: number, c: number): string {
    ...
    // if (a === b && b === c) {
    //     return '10'
    // устал и плохо себя чувствовал когда писал этот код :)
    // } else if (((a === b && a !== c) && (a + b > c && a + c > b && b + c > a)) || ((c === b && b !== a) && (a + b > c && a + c > b && b + c > a))) {
    //     return '01'
...
export function getSum(n: number): number {
    ...
    // let sum = numArr.reduce((a, b) => a + parseInt(b), 0);
    // return sum
...
export function getSquarePositiveIntegers(array: Array<number>): Array<number> {
    ...
    // return array.filter(n => n > 0 && Number.isInteger(n)).map(n => n * n)
    ...
    return array.filter(n => n > 0 && !(n % 1)).map(n => n ** 2)
}
